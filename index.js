...
function genQnParam(Quality = quality) {
    return 'qn=' + Quality + (Quality == QUAILITY_TABLE['4K'] ? '&fourk=1' : '');
}
...
function updateBiliUrl(Query = query) {
    if (Query.length) {
        ...
        if (Query) {
            ...
            url = (vType == 'av' && /[^0-9]/.test(Query)) ?
                null : ("https://b23.tv/" + vType + Query);
...
const copyText = (id) => {
    ...
    if (supportCopy) {
        // 支持复制
        const copyReact = (e) => {
            e.clipboardData.setData('text/plain', document.querySelector('textarea#' + id).value);
            e.preventDefault();
        }
        ...
        try {
            ...
            document.querySelector('button#copy-' + id).classList.add('btn-success');
            setTimeout(() => {
                document.querySelector('button#copy-' + id).innerText = '复制';
                document.querySelector('button#copy-' + id).classList.remove('btn-success');
            }, 500);
        } catch (err) {
            supportCopy = false;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (document.querySelector('input[name=query]').oninput !== undefined)
        ...
        document.querySelector('input[name=query]').onpropertychange = autoShowGoBiliBtn;
    // 复制
    ...
    // JS输出
    updateOutput();
    document.querySelector('#output-evolved #evolved').value = zipTxt(genEvalStr.evolved());
})
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    footerDom.style.transform =
        'translateY(' + (
            ...
            0 :
            document.body.clientHeight - footerDom.offsetTop - footerDom.clientHeight
        ) + 'px)';
    //}, 1);
})