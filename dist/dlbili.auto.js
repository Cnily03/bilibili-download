...
const dlbiliAuto = async (videoQuality, enforceRPC = false) => {
    ...
    const dl_rpc = async (rqUrl, rqMethod, urlArr, filename) => {
        for (let i = 0; i < urlArr.length; i++) {
            ...
            let partIndex = i
                .toString()
                .padStart(urlArr.length.toString().length, "0");
            var rqData = {
                ...
                params: [
                    ...
                    {
                        out: `${filename}${
                            urlArr.length - 1 ? ` Part ${partIndex}` : ""
                        }${suffix}`,
            ...
            // if GET
            if ("GET" == rqMethod)
                rqData.params = cryptoBase64(
                    unicodefy(JSON.stringify(rqData.params))
                );
            await new Promise(resolve => {
                $.ajax({
                    url: rqUrl,
    ...
    const dl_page = async (page = 1, isReturn = false) => {
        ...
        let partname = legalFilename(
                moreThanOneP ? vInfoPages[page - 1].part : vTitle
            ),
            filename = `${bvid}${
                moreThanOneP
                    ? `_p${page.toString().padStart(pagesStrLen, "0")}`
                    : ""
            } ${partname}`;
        if (isReturn) {
            return [urls, filename];
        } else {
            dl_urls(urls, filename, partname);
            ...
            do {
                ...
                if (_rpcMethod != null) {
                    ["POST", "GET", ""].includes(_rpcMethod.toUpperCase())
                        ? ((_rpcMethod = _rpcMethod || rpcMethod),
                          (isContinue = 0))
                        : (isContinue = 1);
                } else isContinue = 0;
            } while (isContinue);
    ...
    const logPages = () => {
        const consoleStyles = {
            ...
            partCur:
                "color:#FFF;background-color:#C63;border-radius:20%;margin-left:10px;",
        };
        console.clear();
        console.info("%c 分P列表 ", consoleStyles.title);
        vInfoPages.forEach(ele => {
            console.info(
                `%c ${ele.page.toString().padStart(pagesStrLen, "0")} %c ${
                    ele.part
                } `,
                ele.page == curPage
                    ? consoleStyles.pageCur
                    : consoleStyles.page,
            ...
            if (inputVal == "") dl_page(curPage);
                ...
                if (dlPages.length && flag) {
                    if (dlPages.length <= CONFIG.MAX_DOWNLOAD_PAGES) {
                        ...
                        dlPages.length - 1
                            ? dl_morePages(dlPages)
                            : dl_page(dlPages[0]);
                    } else {
                        autoPrompt(
                            `为了您的账号安全，单次下载请不要超过${CONFIG.MAX_DOWNLOAD_PAGES}集。\n\n`
                        );
                    }
                } else {
